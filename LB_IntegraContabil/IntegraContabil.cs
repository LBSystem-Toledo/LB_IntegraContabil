...
namespace LB_IntegraContabil
{
    public class IntegraContabil
    {
        ...
        {
            tmpTarefa = new Timer();
            tmpTarefa.Interval = 14400000;//Cada 4 horas
            tmpTarefa.Elapsed += TmpTarefa_Elapsed;
            tmpTarefa.Start();
        }

        private async void TmpTarefa_Elapsed(object sender, ElapsedEventArgs e)
        {
            tmpTarefa.Stop();
            try
            {
                ...
                {
                    string conexao = ConfigurationManager.AppSettings[ConfigurationManager.AppSettings.AllKeys[i]];
                                    ...
                                    if (emp.Dt_expira < DateTime.Now || string.IsNullOrWhiteSpace(emp.Token) || string.IsNullOrWhiteSpace(emp.Integration_key))
                                    {
                                        try
                                        {
                                            ...
                                            if (token != null)
                                            {
                                                ...
                                                IntegrationKey key = await DataService.GerarKeyIntegracaoAsync(emp, config.Url_dominio);
                                                if (key != null)
                                                {
                                    ...
                                    if (token_valido)
                                        if (string.IsNullOrWhiteSpace(p.Code))
                                        {
                                            ...
                                            else if (p.Tp_nota.Trim().ToUpper().Equals("P"))//Nota Propria
                                            {
                                                ...
                                                if (loteNFe != null && !string.IsNullOrWhiteSpace(p.Xml_nfe))
                                                {
